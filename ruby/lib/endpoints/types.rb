...
def call(method, path, ret_type, ret_is_list = false, data = nil, params = nil, api_version = 1)
  ...
  if ret_type.nil?
    ...
    return ApiList.from_json_dict(self.class, ret, self, ret_type)
  elsif ret.is_a? Array
    res = []
    ret.each do |x|
      #res << ret_type.from_json_dict(x, self)
      res << ret_type.from_json_dict(self.class, x, self)
    end
    return res
  else
    #return ret_type.from_json_dict(ret, self)
    return ret_type.from_json_dict(self.class, ret, self)
  end
end
...
# TODO: make sure this works
  ...
  def self.from_json_dict(cls, dic, resource_root)
    ...
    pp dic
    puts "RR"
    pp resource_root.class
    pp resource_root
    obj = cls.new(resource_root)
